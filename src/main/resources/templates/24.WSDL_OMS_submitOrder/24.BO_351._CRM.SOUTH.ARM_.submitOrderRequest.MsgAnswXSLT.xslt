...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="submitOrderResponse!=''">
				<Confirmation>
					<ResultCode>
						<xsl:value-of select="/submitOrderResponse/orderResult/orderResultCode"/>
					</ResultCode>
					<Message>
						<xsl:value-of select="/submitOrderResponse/orderResult//orderResultText"/>
					</Message>
					<DetailList>
						<xsl:copy-of select="submitOrderResponse"/>
					</DetailList>
					<Next>RESOUT</Next>
				</Confirmation>
			</xsl:when>
