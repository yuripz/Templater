	...
	<xsl:template match="/">
					...
					<tns:submitOrderResponse xmlns:tns="http://oms.rt.ru/">
						...
						<xsl:for-each select="order">
							<order>
								<acknowledgement>
									<orderId>
										<xsl:value-of select="acknowledgement/orderId"/>
									</orderId>
									<orderOMSId>
										<xsl:value-of select="acknowledgement/orderOMSId"/>
									</orderOMSId>
									<orderOMSurl>
										<xsl:value-of select="acknowledgement/orderOMSurl"/>
									</orderOMSurl>
									...
									<orderState>
										<xsl:value-of select="/Confirmation/submitOrderResponse/order/OrderResult/orderState"/>
									</orderState>
									<!--									<xsl:choose>
		...
		<xsl:if test="Result!=''">
			<tns:Fault xmlns:tns="http://oms.rt.ru/">
				...
				<Message>возврат из прикладного обработчика был произведен ранее, чем был сформирован ответ. —бой синхронизации потоков
				</Message>
			</tns:Fault>
		</xsl:if>
	</xsl:template>
