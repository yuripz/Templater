...
//                        rs.getString("Log_Level"),
    ...
    public static  int SelectMsgTypes(Logger AppThead_log )  {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                log.info(" Types.size=" + MessageType.AllMessageType.size() + ", MessageRowNum[" + MessageType.RowNum + "] interface_id="+ messageTypeVO.getInterface_Id()+ " :" + messageTypeVO.getMsg_Type() );
            }
        } catch (Exception e) {
            e.printStackTrace();
            return -2;
    ...
    public static  int SelectMsgTemplates(Logger AppThead_log )  {
        ...
        String TemplateS_FILTER = System.getenv("TemplateS_FILTER");
        if ( TemplateS_FILTER == null )
             TemplateS_FILTER = "and (1=1)";
        String selectMsgTemplate = "select * from (select t.template_id, " +
                "t.interface_id, " +
                "t.operation_id, " +
                "t.msg_type, " +
                "t.msg_type_own, " +
                ...
                "where (1=1) " + TemplateS_FILTER + //and t.template_dir like '%OUT%'\n" +
                // "and t.operation_id =0 " +
                //     " and t.interface_id in(26, 24, 19, 15) " +
                " order by t.interface_id, t.operation_id, t.destin_id, t.dst_subcod ) MsgTemplate";
        ...
        if ( DataAccess.Hermes_Connection != null )
            try {
                ...
                stmtMsgTemplate = DataAccess.Hermes_Connection.prepareStatement( selectMsgTemplate);
        ...
        try {
            log.warn("selectMsgTemplate:"  + selectMsgTemplate );
            rs = stmtMsgTemplate.executeQuery();
            while (rs.next()) {
                MessageTemplateVO messageTemplateVO = new MessageTemplateVO();
                messageTemplateVO.setMessageTemplateVO(
...
//                        rs.getString("Log_Level"),
                ...
                "INSERT INTO " + DataAccess.HrmsSchema + ".MESSAGE_TEMPLATES (TEMPLATE_ID, INTERFACE_ID, OPERATION_ID, MSG_TYPE, MSG_TYPE_OWN, TEMPLATE_NAME, TEMPLATE_DIR, SOURCE_ID,  SRC_SUBCOD, DESTIN_ID, DST_SUBCOD, LASTMAKER, LASTDATE)" +
                        "VALUES (" +
                        rs.getInt("template_id") + ", " +
                        rs.getInt("Interface_Id") + ", " +
                        rs.getInt("Operation_Id") + ", '" +
